...
// @checkurl       http://ss-o.net/userjs/twittermultiplayer.user.js
// @lastupdate     2008/01/29 15:12:00
// @version        0.2
// ==/UserScript==

// based on Twitter_foto_expander (http://d.hatena.ne.jp/ono_matope/20071022#1193049261)

(function(){
	var doing = $x('//table[@class="doing"]|id("timeline")');
	if (!doing) return;
	...
	var PHOTO_INFO = {
		...
		,twitter:{
			 url:'^http://twitter\\.com/(\\w*?)\/statuses\/(\\d+)'
	...
	function getElementsByXPath(xpath, node) {
		...
		var nodesSnapshot = node.evaluate(xpath, node, null, XPathResult.ORDERED_NODE_SNAPSHOT_TYPE, null);
		var data = [];
	...
	function getFirstElementByXPath(xpath, node) {
		//log('getFirstElementByXpath(\''+xpath+'\','+node+')');
		var node = node || document;
		var result = node.evaluate(xpath, node, null,XPathResult.FIRST_ORDERED_NODE_TYPE, null);
		var res_value=result.singleNodeValue ? result.singleNodeValue : null;
		//log('result: '+res_value);
		return res_value;
	}
...
*/
		...
		,jsonpLoader:function(opt){
			...
			window[gm] = {};
			window[gm][factory] = {};
			window[gm][factory][callback] = function(obj){
	...
	Object.extend(Fotolife.prototype,Photo.prototype);
	...
	Object.extend(Flickr.prototype,Photo.prototype);
	...
	Object.extend(Ustream.prototype,Photo.prototype);
	...
	Object.extend(YouTube.prototype,Photo.prototype);
	...
	Object.extend(Niconico.prototype,YouTube.prototype);
	Niconico.prototype.load = function(){
		var self = this;
		var ifm = document.createElement('iframe');
		ifm.frameborder = 'none';
		ifm.scrolling = 'no';
		Object.extend(ifm,self.info.size);
		ifm.src = this.href;
		self.element.appendChild(ifm);
		self.success();
	};
	Object.extend(Kichiku.prototype,Fotolife.prototype);
	if (!window.opera) {
		Kichiku.prototype.load = Kichiku.prototype.gmxloader;
		Object.extend(Hirameki.prototype,Kichiku.prototype);
	}
	Twitter.prototype = {
	...
	Object.extend(Twitter.prototype,Photo.prototype);
	if(window.opera) {
	...
	var loader = function(anchr){
		for (var id in PHOTO_INFO) {
			...
			info.id = id;
			var resexp, exp = new RegExp(info.url);
			if ( (resexp = anchr.href.match(exp))) {
	...
	var FotoLoader = function(page) {
		var anchor = $x('descendant::*[contains(@class,"entry_content")]/a',page);
		if (anchor) {
			var metas = $x('descendant::*[contains(@class,"entry-meta")]/a[not(@class="entry-date")]',page);
			if (metas) anchor = anchor.concat(metas);
			Array.each(anchor,loader);
		}
	}
	FotoLoader(doing[0]);
	var top = $x('//div[contains(@class,"desc hentry")]');
	if (!!top) FotoLoader(top[0]);

	addFilter(function(docs) {
		Array.each(docs,FotoLoader);
	});
	...
	if (window.Minibuffer) {
		...
		var openLinks = {
			...
			,command: function(stdin) {
				forEach(stdin,function(para,i) {
					var players = $x('descendant::*[contains(@class,"entry_content")]/a',para) || [];
